...
$( document ).on( "pagebeforeshow", "#postIndex", function() {
	...
	var url = "http://123malaysia.com/tamilsol/?json=1&count=2000&include=title,content,date,thumbnail";
...
$( document ).on( "pagebeforeshow", "#postPage", function() {
    ...
    var postid = getQueryVariable("id");
    var url = "http://123malaysia.com/tamilsol/api/get_post/?post_id="+postid+"&callback=?";
...
function posts(json) {
  	...
  	for (var i = 0; i < json.posts.length; i++)
	{
	   ...
	   if ("title" in json.posts[i]) {
        ...
        html = html + "<li>" + "<small>" + posttitle + "</small>" + "<h1>" + postcontent + "</h1>" + "</li>";
